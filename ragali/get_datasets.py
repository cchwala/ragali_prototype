...
def tranform_fencl_2021_Eband_data(fn):
    ...
    with zipfile.ZipFile(fn) as zfile:
        ...
        f = zfile.open(
            "data_metedata/commercial_microwave_link_total_loss/cml_total_loss.csv")
        ...
        ds = xr.Dataset(
            data_vars={
                'tl': (
                ('sublink_id', 'time'), [df_data.s73.values, df_data.s83.values]),
...
def transform_fencl_2020_Eband_data(local_path, fn):
    ...
    with zipfile.ZipFile(local_path + fn) as zfile:
        ...
        df_metadata = pd.read_csv(
            zfile.open(
                'raw/commercial_microwave_links_total_loss/metadata_table_commercial_microwave_links.csv'),
            index_col=0,
            sep=';',
        )
        ...
        for i, row in df_metadata.iterrows():
            for ab in ['a', 'b']:
                raw_data_fn = os.path.join('raw/commercial_microwave_links_total_loss',
                                           f'{i}{ab}.csv')
...
def transform_overeem_2019_large_CML_data_Netherlands(fn, nrows=None):
    ...
    for cml_id in np.unique(df2012.id):
        ...
        ds_list.append(
            xr.Dataset(
                data_vars=dict(
                    rsl_min=("time", df_sel.pmin.values),
                    rsl_max=("time", df_sel.pmax.values)
                ),
                coords=dict(
...
def transform_andersson_2022_OpenMRG(
    ...
    if restructure_data == True:
        ...
        df_metadata = df_metadata.set_index(['Direction', 'Link']).sort_values(
            [('Sublink')], ascending=True)
        ds_multindex = ds.assign_coords({'sublink': df_metadata.index})
        ds_multindex = ds_multindex.unstack()
        ds_multindex = ds_multindex.rename(
            {'Direction': 'sublink_id', 'Link': 'cml_id'})
        ds_multindex['polarization'] = xr.where(
            ds_multindex['polarization'] == 'Vertical', 'v', 'h')
        ds_multindex['sublink_id'] = xr.where(ds_multindex['sublink_id'] == 'A',
                                              'sublink_1', 'sublink_2')
        ...
        ds_multindex = add_cml_attributes(
            ds_multindex)  # add all attributes afer restructure
    ...
    if restructure_data == True or resample_1min == True:
        ...
        ds_cml2chl = xr.Dataset(
            ...
            coords=dict(
                ...
                sublink=(
                ('cml_id', 'sublink_id'), df_metadata.Sublink.values.reshape(-1, 2)),
        ...
        for cml in ds.sublink:
            ...
            cml_name = \
            df_metadata.loc[df_metadata['Sublink'] == cml.values].index.values[0]
            sublink_channel = df_metadata.loc[
                df_metadata['Sublink'] == cml.values].Direction.values
            sublink_channel = \
            xr.where(sublink_channel == 'A', 'sublink_1', 'sublink_2')[0]
            ...
            if resample_1min:
                ds_rsl = ds.sel(sublink=cml, time=slice(
                    ...
                    "1T").mean(
                    numeric_only=True).to_xarray().rsl  # numeric_only: skip non-numeric
